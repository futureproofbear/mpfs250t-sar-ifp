# PF_CCC_C0_95.tcl -- 95 MHz fabric / 11.875 MHz CoreFFT SLOWCLK.
#
# PREPARED 2026-07-31 as the fallback if the 100 MHz domain cannot be closed with both 32-tap sinc
# kernels present. The critical path is COEFG's fp32 multiplier (kr * tan_s), which predates the
# sinc work -- the interpolators are not on ANY reported timing path. A 5% clock reduction buys
# ~5% setup margin without touching a line of datapath.
#
# COST: the frame scales with the clock. 14.166 s -> ~14.9 s, still inside the 15 s budget, but the
# margin drops to ~0.1 s. Both the fabric stages and the FFTs slow down, because SLOWCLK is kept at
# fabric/8 -- the ratio the CoreFFT CDC constraints (sar_fft_cdc.sdc) are written against, so it is
# deliberately NOT decoupled to hold 12.5 MHz.
#
# PREFERRED ORDER: retiming (free, latency-preserving) -> this -> hand-pipelining sar_fp32_mul
# (keeps 100 MHz and 14.17 s, but needs every emit-pipeline alignment re-derived).
#
# The existing PF_CCC_C0_62p5.tcl is NOT a usable fallback here: 62.5 MHz scales the frame 1.6x to
# ~22.7 s and blows the budget outright.
