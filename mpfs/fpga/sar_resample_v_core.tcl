# sar_resample_v_core.tcl -- register the hand-written resample (sar_resample_v_top wrapping
# sar_resample_v) as an HDL+ core with the SAME bus interfaces the SmartHLS `resample_top` had, so
# sartop_assembly.tcl needs only the hdl_core_name swapped at line 16 and every existing connect
# still lands: RES:axi4initiator -> DIC:AXI4minitiator3, CIC:AXI4mtarget3 -> RES:axi4target.
#
# Like the corner-turn (and unlike the read-only feeder / write-only unloader) this kernel is BOTH
# a read and a write initiator, so its axi4initiator carries AR/R *and* AW/W/B. Mirrors
# corner_turn_v_core.tcl otherwise.
...
catch { create_hdl_core -file "$here/sar_resample_v_top.v" -module {sar_resample_v_top} -library {work} }

# ---- axi4initiator: AXI4 master, READ + WRITE ----
catch { hdl_core_add_bif -hdl_core_name {sar_resample_v_top} -bif_definition {AXI4:AMBA:AMBA4:master} -bif_name {axi4initiator} -signal_map {} }
foreach {b c} {
...
} { catch { hdl_core_assign_bif_signal -hdl_core_name {sar_resample_v_top} -bif_name {axi4initiator} -bif_signal_name $b -core_signal_name $c } }

# ---- axi4target: AXI4 slave (control regs) ----
# NOTE the 6-bit AWADDR/ARADDR: the SmartHLS resample_top used 5, which is enough for its four
# HLS_ARG registers but NOT for this core's tables at 0x2c/0x30. See sar_resample_v_top.v.
catch { hdl_core_add_bif -hdl_core_name {sar_resample_v_top} -bif_definition {AXI4:AMBA:AMBA4:slave} -bif_name {axi4target} -signal_map {} }
foreach {b c} {
...
} { catch { hdl_core_assign_bif_signal -hdl_core_name {sar_resample_v_top} -bif_name {axi4target} -bif_signal_name $b -core_signal_name $c } }

puts "SAR_RESAMPLE_V_CORE_DONE"
