...
- `docs/fpga/SAR_ARCHITECTURE_REPORT.md` — the as-built pipeline, block usage, per-stage timing.
...
- Latency reduction (baseline is a bring-up ~120–160 s, not optimized): multi-hart CPU FFT (~4×);
  faster resample (the per-line coefficient geometry + per-line L2 flush is a large hidden cost —
  parallelize across harts and/or make the fabric coherent); coherent fabric (removes all pipeline
  cache flushes); return the FFT fully to fabric if the toolchain butterfly bug is resolved.
